...
<style>
  ...
  * { box-sizing:border-box }
  ...
  <ol>
    ...
    <li>Введите шестизначный код в админке — раздел «Безопасность». Пока код не введён, второй фактор не включён и вход работает по-старому.</li>
...
const run = async () => {
  ...
  console.log(`Ключ выпущен и ждёт подтверждения. Страница: ${OUT}`);
  console.log('Пока код с телефона не введён, вход работает как раньше — запереть себя нельзя.');
  console.log('После сканирования введите код в админке, разделе «Безопасность», и сохраните коды восстановления.');
};
