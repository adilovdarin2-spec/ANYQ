...
export default function App() {
  ...
  /**
   * Сессия кончилась — по любой из причин, по которым она кончается.
   *
   * Так бывает не от поломки: владелец поменял человеку роль, отключил
   * устройство, удалил сотрудника — или за магазин просто не заплатили, и
   * тариф кончился. Раньше касса этого не замечала и оставалась «в сессии» с
   * мёртвым токеном — на каждое действие отказ, и непонятно, что делать.
   * Теперь она возвращает к PIN и передаёт туда слова сервера.
   *
   * Очередь неотправленных продаж при этом остаётся на месте: она привязана к
   * устройству, а не к сессии, и уйдёт, как только кто-нибудь войдёт. Для
  ...
  /**
   * Сколько ждать досылки смены на выходе.
   *
   * Выход — это передача кассы, и очередь у прилавка ждать не будет. Три
   * секунды: хватает, чтобы один запрос ушёл по любой живой сети, и не
   * настолько, чтобы кассир решил, что касса зависла. Не успели — выходим всё
   * равно, смена уйдёт следующей попыткой, как уходила и раньше.
   */
  const LOGOUT_FLUSH_MS = 3000;

  async function handleLogout() {
    /* Смена уходит под тем, кто её открыл, — пока его вход ещё жив.

       Автор чека берётся у смены (сервер, `/pos/sales`), а автор самой смены —
       у того, чей токен её донёс. Донести её может сменщик: касса отправляет
       смену при открытии и повторяет, но если в то утро сети не было, смена
       лежит локально до вечера.

       Смена, оставшаяся локальной, хуже смены с неверным автором: её чеки
       остаются без ссылки и выпадают из сверок вовсе — у кассира выходит
       излишек, которого он не объяснит. Поэтому здесь и пробуем.
    ...
    try {
      ...
      // Сети нет или сервер молчит. Выход это не отменяет: человек уходит
      // домой, а запереть его за кассой значит сделать хуже всем.
    }
    saveSession(null);
    setSession(null);
  }
  ...
  async function handleCreateReturn(payload: {
    ...
    try {
      ...
      /* Сколько из этого ушло из ящика — числом от сервера, а не выводом из
         способа.

         Способа не хватает: у чека, оплаченного наличными и картой пополам, он
         `mixed`, и запись с таким способом счёт ящика не берёт вовсе — кассир
         увидел бы недостачу ровно на отданные наличные. По чеку в долг, наоборот,
         из ящика не выдают ничего, и записи быть не должно.

         Старый сервер числа не пришлёт: касса и сервер выкатываются по
         отдельности. Тогда считаем по способу, как считалось раньше. */
      const ушлоИзЯщика =
        made.cashRefunded ?? (made.paymentMethod === 'cash' ? made.refundAmount : 0);
      if (shift && ушлоИзЯщика > 0) {
        addDrawerEntry({
          id: made.id,
          shiftId: shift.id,
          kind: 'refund',
