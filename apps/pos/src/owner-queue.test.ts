...
describe('ненастроенный магазин', () => {
  /**
   * Первый день.
   *
   * Очередь отвечала «Ничего не требует вашего решения, хороший день»
   * магазину, у которого нет ни одного товара, — то есть который вообще не
   * может торговать. Ответ формально верный: недостач нет, сроков нет, долгов
   * нет. Все они нулевые ровно потому, что продавать нечего.
   */
  it('говорит завести товары, пока их нет', () => {
    const tasks = ownerQueue(EMPTY, NOW, 0);
    expect(tasks.map((t) => t.kind)).toEqual(['empty_catalogue']);
    expect(tasks[0].urgency).toBe('now');
  });

  it('и не говорит этого, когда товары есть', () => {
    expect(ownerQueue(EMPTY, NOW, 12).map((t) => t.kind)).not.toContain('empty_catalogue');
  });

  it('и молчит, если про каталог не спрашивали', () => {
    // Самопроверка: вызов без этого сведения — не повод объявлять магазин
    // пустым. Иначе любой экран, забывший передать число, получит ложное дело.
    expect(ownerQueue(EMPTY, NOW).map((t) => t.kind)).not.toContain('empty_catalogue');
  });

  it('и вытесняет всё прочее, пока держится', () => {
    // Пока товаров нет, остальные дела либо невозможны, либо неинтересны:
    // читать про неликвид в магазине без товаров нечего.
    const busy: OwnerDashboard = {
      ...EMPTY,
      money: { ...EMPTY.money, shifts: [closedShift({ closedAt: null, openedAt: '2026-09-15T09:00:00.000Z' })] },
    };
    const tasks = ownerQueue(busy, NOW, 0);
    expect(tasks.map((t) => t.kind)).toEqual(['empty_catalogue']);
  });
});
