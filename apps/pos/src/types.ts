...
/**
 * Деньги, прошедшие через ящик мимо чека.
 *
 * Таких движений два вида, и оба меняют сумму, которую кассир пересчитывает на
 * закрытии смены:
 *
 *   - **возврат** — деньги ушли покупателю;
 *   - **расчёт** — клиент погасил долг наличными (пришли) или поставщику
 *     заплатили из ящика (ушли).
 *
 * Хранится у кассы, хотя проводится всё это только на сервере: ящик здесь, и
 * без этих записей закрытие смены считает ожидаемую наличность по одним лишь
 * чекам. Кассир, честно пересчитавший ящик, получал недостачу ровно на сумму,
 * которую отдал покупателю, и излишек ровно на ту, которую принял по долгу.
 * Сервер свою половину считает правильно, так что владелец и кассир смотрели
 * на два разных числа — и то, которое видел кассир, обвиняло его.
 */
export interface DrawerEntry {
  /** Идентификатор документа на сервере. */
  id: string;
  /** Смена, через ящик которой прошли деньги. */
  shiftId: string;
  kind: 'refund' | 'settlement';
  /** `in` — деньги пришли в ящик, `out` — ушли из него. */
  direction: 'in' | 'out';
  amount: number;
  /** Чем: ящика касается только наличное. */
  method: PaymentMethod;
  createdAt: string;
}
