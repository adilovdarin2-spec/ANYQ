...
export function addSale(sale: Sale): void {
  ...
  // точно растёт, и стоит это один проход по массиву, который и так
  // переписывается целиком. Смену, которая идёт прямо сейчас, чистка не
  // трогает — по её чекам на закрытии считают деньги в ящике.
