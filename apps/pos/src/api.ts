...
export interface CreateReturnResult {
  ...
  paymentMethod: string;
  /**
   * Сколько из возврата ушло из ящика.
   *
   * Способа для этого не хватает: у чека, оплаченного наличными и картой
   * пополам, он теперь `mixed`, и счёт по способу вычел бы ноль — то есть
   * показал бы кассиру недостачу ровно на отданные наличные.
   *
   * Необязательное: сервер и касса выкатываются по отдельности, и ответ
   * сервера на версию старше этого поля его не несёт. Тогда считается по
   * способу, как считалось раньше.
   */
  cashRefunded?: number;
