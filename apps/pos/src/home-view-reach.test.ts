...
describe('первый экран смены', () => {
  ...
  it('и начальное состояние спрашивает его же', () => {
    // Иначе первый кадр после запуска — чужой экран, и официант видит подскок
    // с кассы на зал.
    const начало = app.indexOf('useState<View>(');
    expect(начало, 'начальное состояние экрана не нашлось').toBeGreaterThan(-1);
    expect(app.slice(начало, начало + 240)).toContain('homeViewFor(');
  });
