// Почему касса — один файл, а не десяток загружаемых по требованию.
//
// 138 КБ в gzip на всё приложение выглядят как повод разбить его на части и
// подгружать экраны по мере надобности. Здесь этого делать нельзя, и причина в
// строке ниже: предзагрузка разбирает HTML и кладёт в кэш то, на что он
// ссылается. Динамический чанк в HTML не упомянут — его адрес становится
// известен в тот момент, когда экран открывают. То есть касса, которую поставили
// и унесли в магазин без связи, откроет продажу и не откроет приёмку.
//
// Обещание «неделю без сети» дороже трёх секунд на первой загрузке, которые
// случаются один раз. Если делить всё-таки понадобится, сначала придётся
// научить предзагрузку брать список файлов из сборки, а не из разметки.
