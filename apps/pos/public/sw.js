// Чего нельзя делать с этой кассой: подгружать экраны лениво.
//
// 138 КБ в gzip выглядят как повод разбить приложение на части. Разбить по
// статическим зависимостям можно — такие файлы Vite упоминает в разметке, и
// предзагрузка ниже их подхватит. А вот `import()` по требованию — нельзя:
// адрес такого файла появляется в ту секунду, когда экран открывают, в HTML его
// нет, и в кэш он не попадёт. Касса, которую настроили и унесли в магазин без
// связи, откроет продажу и не откроет приёмку.
//
// Обещание «неделю без сети» дороже трёх секунд на первой загрузке, которые
// случаются один раз. Если ленивая загрузка всё же понадобится, сначала придётся
