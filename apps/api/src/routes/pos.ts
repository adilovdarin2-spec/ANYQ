...
posRouter.post('/login', loginRateLimit, async (req, res) => {
  const { pin } = req.body ?? {};
  /* PIN — строка, и это проверяется до запроса, а не после.

     Стояло `if (!pin)`, и дальше `pin` уходил в `where: { posPin: pin }` как
     есть. Объект в этом месте Prisma читает не как значение, а как фильтр:
     `{"pin":{"not":null}}` означало «любой непустой PIN», находило первого же
     сотрудника с PIN-ом и выдавало на него рабочий токен кассы. То есть войти
     в чужую кассу мог кто угодно, кто дотянулся до адреса, не зная ничего.

     Проверка типа — и есть починка: строка ничем, кроме значения, быть не
     может. Ответ тот же, что на пустое поле: кассиру, у которого сорвалась
     клавиатура, незачем читать про типы, а тому, кто пробует, незачем знать,
     что его попытку узнали. */
...
/**
 * Утренняя сводка владельца.
 *
 * Замерено на год торговли, чтобы следующий читал числа, а не догадки: 21 900
 * продаж, 800 товаров, 365 дней — сводка отвечает за 67 мс, отчёт за 78 мс.
 * Причина в том, что обе ограничены окном по дате и считаются суммами в базе,
 * а не загрузкой строк в Node, — в отличие от расчётов с контрагентами, где
 * иначе и где это записано рядом с их запросом.
 *
 * Трогать здесь нечего; замер оставлен, чтобы не пришлось мерить снова, когда
 * покажется, что «наверное, тормозит сводка».
 */
...
/**
 * Сотрудники магазина — дело магазина.
 *
 * До 15.09.2026 их заводили только из панели ANYQ. Это значило две вещи, обе
 * плохие. Владелец не мог сам поменять кассиру PIN — звонил нам. А мы держали
 * у себя имена и телефоны чужих сотрудников, хотя тарифы делятся на их
 * количество, а не на их имена.
 *
 * Теперь список ведёт владелец, а панель платформы видит только число. PIN
