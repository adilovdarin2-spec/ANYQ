...
export async function requireAuth(req: AuthedRequest, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    // Строго, без поблажки для токенов без версии, — в отличие от кассы.
    // Касса пускает старые мягко: иначе выкатка выгоняет из смены каждый
    // планшет в стране. Здесь наоборот: токенов до этой правки ровно столько,
    // сколько открытых вкладок у одного человека, а среди них и может быть тот
    // самый чужой, ради которого всё это и делается. Один вход заново — не
    // цена.
