 ...
 *
 * Один такой пересказ существует и существовать обязан: `cartTotals` в кассе.
 * Без него касса не назвала бы сумму без сети, то есть перестала бы быть
 * кассой, а серверу присланному числу верить нельзя — скидку правит менеджер,
 * баллы лежат в базе. Кассы поэтому нет в списке ниже, и держит её не чтение
 * исходника, а `till-and-server-agree` в `scripts/`: он сравнивает ответы обеих сторон на
 * четырёх сотнях входов, включая вес и границы, и переживёт переписывание
 * любой из них.
 */
