...
/**
 * За сколько дней журнал правда прочитан.
 *
 * Расчёт спроса ограничен числом движений: девяносто дней журнала оптовика
 * читать нельзя, а отвалившийся по таймауту экран пополнения хуже короткого
 * окна. Но окно при этом оставалось прежним, и дни, до которых чтение не
 * дошло, выглядели как «товар лежал и ничего не продавалось».
 *
 * Для спроса это худший вид ошибки: он делится на дни, когда товар был в
 * наличии, — а пустые дни выглядят именно такими. Делитель растёт, спрос
 * падает, и совет «заказывать не надо» приходит ровно в тот магазин, где
 * журнал плотнее всего, то есть в самый бойкий.
 *
 * Поэтому окно сужается до прочитанного. Период выходит короче — и правдивый,
 * а короткое честное окно лучше длинного, наполовину состоящего из тишины.
 *
 * Движения приходят от новых к старым, поэтому самое старое — последнее.
 */
export function demandWindowDays(
  movements: { createdAt: Date }[],
  windowDays: number,
  limit: number,
  now: Date,
): number {
  if (movements.length < limit || movements.length === 0) return windowDays;
  const oldestRead = movements[movements.length - 1].createdAt;
  // Тем же счётом, каким движению назначается его день: `floor` от возраста.
  // Движение возрастом ровно трое суток лежит в дне номер три, а дней с
  // нулевого по третий — четыре. `ceil` от возраста терял бы один день и
  // завышал спрос на нём — ошибка меньше прежней, но в другую сторону, и
  // нашлась она только тем, что три функции поставили рядом.
  const oldestDayIndex = Math.floor((now.getTime() - oldestRead.getTime()) / (24 * 60 * 60 * 1000));
  const covered = oldestDayIndex + 1;
  // Не меньше одного дня и не больше запрошенного: сузить окно расчёт вправе,
  // расширить — нет.
  return Math.min(Math.max(covered, 1), windowDays);
}
