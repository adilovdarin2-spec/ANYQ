 ...
 * инвентаризация живут в `stock`. Сервер такой набор не примет
 * (`apps/api/src/modules.ts`), поэтому форма проставляет зависимость сама, а
 * не даёт собрать набор, который потом откажутся сохранить.
 */
