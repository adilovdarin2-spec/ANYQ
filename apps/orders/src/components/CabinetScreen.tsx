    ...
    <div className="cab">
      ...
      {summary && (
        <>
          ...
          {/* Долг принадлежит компании, а не точке: покупатель должен магазину,
              а не его полке на Абая. Всё остальное на этом экране посчитано по
              выбранной точке, и без этой оговорки владелец сети, переключая
              точки, видел бы один и тот же долг на каждой и читал бы его как
              долг каждой. У кого точка одна, разницы нет. */}
          {debts && (debts.receivable.total > 0 || debts.payable.total > 0) && (
            <Section title={locations.length > 1 ? 'Долги по компании' : 'Долги'}>
          ...
          <footer className="cab-foot">
            {/* Обещание должно быть правдой до последнего слова, и его
                приходится переписывать каждый раз, когда за этой дверью
                появляется что-то новое. Сначала здесь стояло «изменить нельзя
                ничего»; потом сюда переехало согласие на доступ поддержки;
                16.09.2026 — PIN-ы сотрудников. Оставить прежний текст было бы
                хуже, чем не писать его вовсе: обещание, которое читатель может
                проверить и опровергнуть тут же, обесценивает и всё остальное,
                что здесь написано.

                Перечисление осталось, потому что суть не изменилась: торговля
                через кабинет не меняется. Ни товар, ни цена, ни остаток, ни
                продажа — за них по-прежнему отвечает касса. */}
            Товар, цену, остаток и продажи кабинет только показывает — изменить их отсюда
            нельзя. Решается в нём два: пускать ли ANYQ посмотреть эти цифры, когда мы просим
            помочь, и кто из ваших людей входит в кассу. Второе — за отдельным замком.
          </footer>
        </>
      )}
    </div>
